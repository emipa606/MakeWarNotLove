...
namespace RimWorld
{
	...
	public class InteractionWorker_MarriageProposalMWNL : InteractionWorker
	{
		...
		public virtual void Interacted(Pawn initiator, Pawn recipient, List<RulePackDef> extraSentencePacks)
		{
			...
			bool flag4 = initiator.IsColonist || recipient.IsColonist;
			if (flag4)
			{
		...
		private void SendLetter(Pawn initiator, Pawn recipient, bool accepted, bool brokeUp)
		{
			...
			if (accepted)
			{
				text = Translator.Translate("LetterLabelAcceptedProposal");
			...
			{
				text = Translator.Translate("LetterLabelRejectedProposal");
			...
			Find.LetterStack.ReceiveLetter(text, stringBuilder.ToString().TrimEndNewlines(), letterDef, initiator, null);
		}
